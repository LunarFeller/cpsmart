# cpsmart

cpsmart 是一款轻量、原生、完全本地运行的 macOS 剪贴板历史工具。它记录文本、图片和文件复制历史，通过 `⇧⌘V` 在当前屏幕底部打开横向历史窗口。

## 主要功能

- 本地记录文本、图片和文件，不联网、不上传历史。
- 打开后默认浏览卡片，按 `Tab` 进入或退出搜索。
- 使用 `←` / `→` 选择，空格键通过 Quick Look 预览，回车直接粘贴。
- 鼠标单击卡片选择并复制，双击返回原应用并粘贴。
- 按全部、文本、图片和文件筛选，支持忽略大小写、变音符号和字符宽度的搜索。
- 显示图片缩略图、来源应用名称与图标。
- 使用 `⌘P` 置顶；置顶记录不会被普通清空、数量上限或保留期限删除。
- 支持浅色、深色和跟随系统外观。
- 自动跳过带标准隐藏、临时或自动生成标记的剪贴板内容。
- 支持暂停记录和登录时启动。

历史默认保留 200 条、最多约 25 MB，单张图片最大 8 MB。数据保存在：

```text
~/Library/Application Support/cpsmart/history.json
```

## 安装

1. 从 GitHub Releases 下载 `cpsmart-<版本>-universal.dmg`。
2. 打开 DMG，把 `cpsmart.app` 拖到 `Applications`。
3. 从 Finder 的“应用程序”中启动 cpsmart，菜单栏会出现剪贴板图标。

macOS 安装应用后不会自动在桌面创建图标。如果需要桌面快捷方式，请在 Finder 的“应用程序”中右键 cpsmart，选择“制作替身”，再把替身拖到桌面；不要把应用本体移出“应用程序”。

### 未签名测试包

本地构建使用 ad-hoc 临时签名，只适合开发测试。第一次打开时，可以先尝试右键应用并选择“打开”；如果系统仍阻止启动，请前往“系统设置 → 隐私与安全性”，在“安全性”区域选择“仍要打开”。不建议运行来源不明的“解除限制”脚本，也不要全局关闭 Gatekeeper。

面向普通用户发布时，应使用 Developer ID 签名并完成 Apple 公证。

## 辅助功能权限

回车或双击粘贴需要向原应用发送一次 `⌘V`，因此需要 macOS“辅助功能”权限：

1. 打开“系统设置 → 隐私与安全性 → 辅助功能”。
2. 点击应用列表下方的 `+`。
3. 从“应用程序”中选择 cpsmart，并打开右侧开关。
4. 完全退出 cpsmart，然后重新打开。

临时签名会随构建变化。安装新的本地测试包后，如果权限失效，请在该页面用 `−` 删除旧项，再用 `+` 重新添加 `/Applications/cpsmart.app`。

## 使用方式

| 操作 | 键盘 | 鼠标 |
|---|---|---|
| 打开或关闭历史 | `⇧⌘V` | 菜单栏“打开剪贴板历史” |
| 选择并复制 | `←` / `→` | 单击卡片 |
| 粘贴 | `Return` | 双击卡片 |
| Quick Look 预览 | `Space` | — |
| 搜索/返回浏览 | `Tab` | 点击搜索框/搜索框外 |
| 置顶 | `⌘P` | — |
| 删除所选 | `⌘⌫` | — |
| 类型筛选 | `⌘1`–`⌘4` | 点击类型筛选器 |
| 清除搜索或关闭 | `Esc` | — |

普通“清空历史”会保留置顶记录。按住 `⌥` 打开菜单，可选择“清空全部历史（含置顶）”。

## 开发与测试

要求 macOS 13 或更高版本，以及 Apple Command Line Tools。

```bash
swift build
bash Scripts/run_tests.sh
```

## 构建安装包

版本号分别保存在 `VERSION` 和 `BUILD_NUMBER`。发布新版本前更新这两个文件和 `CHANGELOG.md`。

本地测试包：

```bash
bash Scripts/build_dmg.sh --local
```

脚本使用 Release 优化分别构建 Intel 与 Apple Silicon 版本，合并成 Universal 2 应用，并输出到：

```text
dist/cpsmart-<版本>-universal.dmg
```

正式签名和公证包：

```bash
bash Scripts/build_dmg.sh --release \
  --sign-identity "Developer ID Application: <名称> (<Team ID>)" \
  --notary-profile "cpsmart-notary"
```

正式模式会拒绝临时签名，并使用 `notarytool` 提交 DMG、等待结果和装订公证票据。签名证书和公证凭据必须保存在开发者的钥匙串中，不能提交到仓库。

## 发布流程

1. 更新 `VERSION`、`BUILD_NUMBER` 和 `CHANGELOG.md`。
2. 运行编译和测试。
3. 构建并验证正式 DMG。
4. 提交发布改动并创建带说明的 Tag，例如 `v1.7.0`。
5. 推送分支和 Tag，在 GitHub 创建 Release。
6. 上传 DMG，粘贴对应版本的更新日志和本页安装说明。

## 参与贡献

Fork、分支、测试和 Pull Request 说明见 [CONTRIBUTING.md](CONTRIBUTING.md)。GitHub 使用 Pull Request（PR）；Merge Request（MR）是 GitLab 的称呼。

## 隐私说明

剪贴板历史可能包含隐私信息。cpsmart 会跳过带标准敏感标记的内容，但并非所有应用都会正确设置这些标记。处理敏感数据时，请暂停记录并定期清空历史。
