...
ClipShelf 是一个轻量的原生 macOS 菜单栏剪贴板历史工具。它在本机记录复制过的文本、图片和文件，按 `⇧⌘V` 会在当前屏幕底部弹出横向历史记录。

## 功能

- 自动记录文本、图片和文件复制历史
- `⇧⌘V` 全局快捷键呼出底部浮窗
- 移动鼠标、单击卡片或按 `←` / `→` 都可选择；选中卡片带有 Dock 式放大效果
...
1. 打开 `dist/ClipShelf-1.2.0-universal.dmg`。
...
第一次使用“回车直接粘贴”时，macOS 会请求“辅助功能”权限。请在“系统设置 → 隐私与安全性 → 辅助功能”中允许 ClipShelf；该权限只用于向你原来所在的应用发送一次 `⌘V`。
